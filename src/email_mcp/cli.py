...
def configure_output(
    ctx: typer.Context,
    json: bool = typer.Option(False, "--json", help="Output JSON (default)."),
...
def _ndjson_lines(payload: dict) -> list[str]:
    ...
    if list_keys:
        key = next((k for k in list_keys if k in {"data", "accounts", "job_ids"}), list_keys[0])
...
def search_hybrid(query: str, account: str | None = None, limit: int = 20, vector_limit: int = 10) -> None:
